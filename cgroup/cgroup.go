...
// by systemd.
func NewDefaultFS(logger log.Logger) (FS, error) {

	mode, err := cgUnifiedCached(logger)
...
func cgUnifiedCached(logger log.Logger) (MountMode, error) {
	...
	case cgroup2SuperMagic:
		level.Debug(logger).Log("Found cgroup2 on /sys/fs/cgroup/, full unified hierarchy")
	...
	case tmpFsMagic:
		...
		if err == nil && fs.Type == cgroup2SuperMagic {
			level.Debug(logger).Log("Found cgroup2 on /sys/fs/cgroup/unified, unified hierarchy for systemd controller")
		...
		switch fs.Type {
		case cgroup2SuperMagic:
			level.Debug(logger).Log("Found cgroup2 on /sys/fs/cgroup/systemd, unified hierarchy for systemd controller (v232 variant)")
			return MountModeHybrid, nil
		case cgroupSuperMagic:
			level.Debug(logger).Log("Found cgroup on /sys/fs/cgroup/systemd, legacy hierarchy")
